...
• Sort recipes by: Name (String) , Cooking time
(int) , Calories (double)
• Display all recipes

Functions and Interactions Menu-driven interface (Scanner + loop):
Polymorphism:
Calling displayRecipe() on a Recipe reference will
automatically use the correct subclass implementation
(DinnerRecipe or DrinkRecipe).
• Add Dinner Recipe
• Add Drink Recipe
• Display All Recipes

Sorting:
Sorting will be implemented using Comparator and
Collections.sort(). which will sort based on the custom
rules user provide.
...
Data Types 
• String (recipe name, cuisine type, fruit
type, nutritional value, etc.)
...
• Each recipe will be displayed with
neat formatting.
• Examples
